...
export const defaultJoinRole: MemberRole = "Brand";

/**
 * Step 1 hands the chosen role to Step 2 through the URL. Anything not in the catalog is
 * discarded rather than echoed back into the page or sent to the API.
 */
export function resolveJoinRole(value: string | null | undefined): JoinRole {
  const match = joinRoles.find((role) => role.name === value);
  return match ?? joinRoles.find((role) => role.name === defaultJoinRole) ?? joinRoles[0];
}

export function isKnownJoinRole(value: string | null | undefined): boolean {
  return joinRoles.some((role) => role.name === value);
}
